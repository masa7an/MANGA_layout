...
def is_binary(mask: QImage) -> bool:
    """濃淡が 0 と 255 だけか。**自動テストが使う確認。**

    保存形式は 0〜255 を受け入れるが、**いま作れるのは白と黒だけ**
    （→ 4.2、`wand._mask_image`）。中間値が混じり始めたことに気づくための
    確認で、それを当てにしている所（→ `EditorState.mask_hides` は 0 だけを
    「消えている」と読む）が黙って狂わないようにしてある。

    **本番の経路からは呼ばない。** 掛けるたびに全画素を見るには重く、
    掛かるかどうかは寸法で決まる（→ `apply_mask`）。
