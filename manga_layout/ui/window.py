...
class MainWindow(QMainWindow):
    ...
    def _hint(self) -> str:
        ...
        if self.state.tool == TOOL_WAND:
            # **出口は道具の名前で言う**（本人の指示 2026-08-27）。ラフ・トーン
            # 範囲と同じ「同じ項目をもう一度押すと戻る」だと、**どの項目のことか
            # 書いていない**——入り口を覚えていない人には辿れない。
            #
            # 名前は `TOOL_LABELS` から引く。ここへ書き写すと、道具を改名した
            # ときに案内だけ古い名前で残る（→ `BALLOON_STYLE_LABELS` の注記）
            return (
                f"切り抜き中: 絵を押すとその区画が消える"
                f" / Shift+押すとそこだけ残す / 許容差 {self.state.wand_tolerance}"
                f" / もう一度、メニュー「{TOOL_LABELS[TOOL_WAND]}」を押すと解除"
            )
