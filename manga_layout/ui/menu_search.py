...
SYNONYMS = {
    ...
    "吹きだし": "フキダシ",
    "ふきだし": "フキダシ",
    # 画面の呼び名は「テキスト」（2026-09-27 に「セリフ」から改めた。
    # フキダシと取り違えるため）。**前の呼び名で打たれても当てる**
    "セリフ": "テキスト",
    "せりふ": "テキスト",
    "びっくり": "ビックリ",
...
_TEXT_FORMAT = "テキストの書式は、大きさ・太字・寄せ・縦横だけです"
...
MISSING_FEATURES = {
    ...
    "手ぶれ補正": _NO_FEATURE,
    "パース定規": _NO_FEATURE,
    # 「マスク」も**ありませんで済ませない**（→ 上の「レイヤー」と同じ）。
    # 中身としては持っているが（絵に掛ける白黒のマスク → 10.3）、
    # 画面に出る呼び名は「切り抜き」なので、その名前を教える
    "マスク": "マスクという名前の項目はありません。絵の一部を消すなら【画像 → 切り抜き】です",
    # 「切り抜き」は**この表から外した**（2026-08-27）。自動領域選択として
    # 実際に入ったので、一覧のほうが道順を答える（→ `WandMenu`）
    "トリミング": _NO_FEATURE,
    "左右反転": _NO_FEATURE,
...
def _walk(
    ...
    for action in actions:
        ...
        if not label:
            continue
        # **メニューバーに直接置かれた、メニューを持たない項目は拾わない。**
        # 辿って実行する道順が無いので、拾うと道順の空欄が並ぶ。
        #
        # 今このページに当てはまる項目は無い。かつての「＋ Alt キー」の案内は
        # 押せない項目から隅の部品へ移し（2026-09-05）、その後廃止した
        # （2026-09-25）。**歯止めは残す**
        # ——項目として飾りを足したくなったときに、黙って検索結果へ混ざらない
        if not path:
            continue
