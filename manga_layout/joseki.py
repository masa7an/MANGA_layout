...
#   0.000〜0.007 が9件（断ち切り）／ 0.014 以上が11件（枠のあるコマ）
# **0.01 はその隙間にある。**（確認日: 2026-09-04。境目はこのとおりだった）
#
# **以前は「13件／7件」と書いていた。** 0.014 未満はサンプル6ページ28コマ全部でも
# 9件しかなく、**13件はどのページを選んでも出せない**（2026-09-05 に発見）。
# （根拠: 5ページ20コマの min(x, y, 1-right, 1-bottom) を正規化後に測った出力
# / 確認日: 2026-09-05）
BLEED_MARGIN = 0.01
