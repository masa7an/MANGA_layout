...
| `reading.py` | **4コマ2列のページかどうか。** 2つある読み順が共有する唯一の部分 |

**読み順は2本ある**（`next_panel.reading_order` と `ui/psd_export.reading_order`）。
段の切り方が違うのは承知のうえで、**共有しているのは `reading.py` の判定だけ**
（→ 要件定義 10.5）。**片方に写して2本持たないこと。**
...
| `text_raster.py` | セリフを1枚の画像に焼く（ラスタライズ → 要件定義 6.34） |

**`render.py` は画面・PNG・PSD の3つから呼ばれる。** 「画面で見た通りに出力される」が
成り立つのはこのため（→ 要件定義 2章）。**描き方を変えると3つ全部に効く**ので、
片方だけ直したいときは分岐ではなく引数で分ける。

**`text_raster.py` はこれに4つめの呼び手として乗る。** セリフを焼くときも
`PageRenderer.draw_text_alone` を通すので、縦書きの組み方・寄せ・書体が画面と
食い違わない。**別に描く経路を作らないこと。**
...
| `hints.py` | 最初の1回だけ出す操作のヒント（ページの上に重ねる帯） |
...
| `history_panel.py` | 履歴パネル（積まれた手を新しい順に並べる。見るだけ） |
