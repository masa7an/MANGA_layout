...
| 症状 | 押す項目 |
|---|---|
| 黒ベタなのにトーンにならない | 「拾う黒を増やす」。JPG や AI で描いた絵は、黒が少し薄くなっていることがあります |
| 線画の線まで斜線になって、絵が壊れた | 「細い線を残す」。押すほど、細いものはトーンにしなくなります |
...
**`Shift`+`]` で広く、`Shift`+`[` で狭く**なります。中心は動かないので、場所を合わせてから
連打で大きさだけを詰められます。何回押しても、元に戻すは1回で済みます。
（このキーが効くのは **トーン範囲を調整** の道具を選んでいる間だけです）
...
アプリを一度起動すると自動で作られるので、メモ帳などで開いて書き換えます。
...
| キー | 動作 |
|---|---|
| `Shift+]` / `Shift+[` | トーンの範囲を広く / 狭く（**トーン範囲を調整の道具を選んでいる間だけ**） |
