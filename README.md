...
**漫画制作支援エディタ**  
ペイントソフト や 生成AI で描いた画像を読み込み、**コマ割り・フキダシ・テキスト配置**を行うためのエディタです。
...
![コマを動かすと、絵もフキダシもテキストも一緒に動く](docs/panel-follow.gif)
...
1. `P` を押してコマを置き、`J`（縦に分割）や右クリックの「横に分割」でコマ割りを作る
2. コマをダブルクリックして中に入り、`Ctrl+V` で画像を貼る（`Ctrl+Shift+F` でコマいっぱいに広げる）
3. `B` / `W` / `G` でフキダシ、`T` でテキストを置く。`F2` で文字を打つ
...
**メニューの ヘルプ → ショートカットキーの一覧...**（`H`）で、キーでできる操作が
メニューごとに並びます。
...
- **テキストを打っている間は、文字の入力が優先されます。** 打ち込んだキーが操作に化けることはありません
...
| フキダシからはみ出したテキスト | 字がフキダシの外へ出ています |
| 空のままのテキスト | 文字を打ち忘れたテキスト |
...
クリスタで開いたとき、コマ枠だけ残す・絵だけ薄くする・テキストを消して打ち直す、が
できるようになります。
...
| 上から | 中身 |
|---|---|
| テキスト | 文字。**クリスタでは打ち直しになります**（文字としては編集できません） |
| マーク | 「！」「!?」 |
| フキダシ | しっぽも含めて |
...
| `favorite_fonts` | 既定の書体1つ | 書体名を3つまで | よく使う書体。道具箱のボタンと `F3` で切り替わります（→ 下記） |

メモ帳などで直接書き換えるときの注意（`settings.bat` の窓から変えるなら、どれも気にしなくて構いません）。

- **フォルダの場所は `\` を2つ重ねて書きます**（`F:\\2025_e\\...`）
- **書体名は1文字でも違うと、黙って別の書体で表示されます**（エラーは出ません）。`settings.bat` の窓から「選ぶ…」で選べば、この間違いは起きません
...
|---|---|
| `V` / `P` | 選択 / コマ追加 |
| `J` / `K` | 縦に分割 / 斜めに縦割り。横に分割はメニューと右クリックから |
| `B` / `W` / `G` | フキダシ（丸い / 雲 / ギザギザ）。トゲトゲ・ふわふわ・四角はメニューと右クリックから |
| `T` / `M` | テキスト / ビックリマーク。ビックリはてなはメニューと右クリックから |
...
| キー | 動作 |
|---|---|
| `F2` / `Enter` | 選択中のテキストの入力欄を開き直す |
| `Ctrl+B` | 太字にする |
| `F7` | 縦書き / 横書きを切り替える |
| `Ctrl+.` / `Ctrl+,` | 大きく / 小さく |
| `F3` | よく使う書体を順に切り替え（下記） |

**テキストを傾けたいときは「画像にする」。** テキストのメニューか右クリックから選ぶと、
そのテキストが**1枚の画像**（透明な背景に字が乗っただけのもの）になります。画像になると
**回せるようになり**、フキダシの外に置く小さな文字を斜めにできます。

- **文字としては打ち直せなくなります。** 直したいときは、消して打ち直してください
- **戻せるのは `Ctrl+Z` だけです。** アプリを閉じたあとは戻りません
- 焼いたあとの扱いはマークと同じです（丸いつまみをドラッグで回転、`Shift` で15度ずつ）
- **字が1つも入っていないテキストは画像にできません**（項目が押せません）

**よく使う書体（`F3`）について。** 書体は200件近く並ぶので、使う3つを登録しておくと、
一覧を開かずに切り替えられます。**登録は `settings.bat` の「よく使う書体」から。**
登録した書体は**画面上の道具箱にボタンとして並び**、今どれを使っているかが押し込みで分かります。
テキストを選んでいれば そのテキストの書体が、選んでいなければ**次に作るテキスト**の書体が変わります。
...
**コードのどこに何があるかは [リポジトリの地図.md](リポジトリの地図.md) にあります。**
層の規則（中核は画面を呼ばない、など）と、各ファイルの役割、新しく足すときに触る場所を
まとめてあります。
