...
class Test読み順の番号:
    ...
    def test_4コマ2列は列優先(self):
        """**4コマ2列のページだけ、右列を上から下・続いて左列**（→ 要件定義 10.5）。

        **2026-09-05 は「行優先のまま揃えない」と決めていた。** 番号はラベルで
        しかなく、揃えると既に書き出した作品の番号が変わるため。**2026-09-06 に
        判断を変えた**——MCP 越しに「2コマ目にセリフを入れて」と言えるように
        なり、番号が指示の宛先になった。ずれると別のコマが喋る。

        **番号が変わるのは、この形のページだけ。** 段の切り方は揃えていないので、
        従来のページの番号は1つも動かない（→ 次のテスト）。
        """
        ...
        assert xs == [638.0] * 4 + [89.0] * 4   # 右列を上から下、続いて左列

    def test_4コマ2列は提案の読み順と一致する(self):
        """**2つの読み順が同じ答えを返す。** 食い違っていた形のページ。

        判定を写さず `reading.column_first` 1本に置いたことを、ここで確かめる
        （写すと片方だけ直って、また食い違う）。
        """
        from manga_layout.next_panel import reading_order as suggest_order

        rects = [Rect(x, y, 500.0, 380.0)
                 for y in (89.0, 504.0, 919.0, 1334.0) for x in (638.0, 89.0)]
        _panels, page = self.make(*rects)
        assert reading_order(page.panels, 35.0) == suggest_order(page.panels)

    def test_3段2列は列優先にしない(self):
        """**4コマ用の特殊な読み方なので、3段には広げない**（→ 要件定義 10.5）。

        左右がぴったり揃った3段のページは、ふつうの漫画のページとして行優先。
        """
        rects = [Rect(x, y, 500.0, 380.0)
                 for y in (89.0, 504.0, 919.0) for x in (638.0, 89.0)]
        _panels, page = self.make(*rects)
        xs = [p.bounds().x for p in reading_order(page.panels, 35.0)]
        assert xs == [638.0, 89.0] * 3          # 行優先
