...
class Test文字の大きさの看板:
    ...
    **看板と実際の割り当てが食い違うと、読んだとおりに押しても動かない。**
    ...
    def test_大きさのキーは入力を確定してから効く(self, window_with_text):
        """**押すと入力から抜ける。これは仕様として許容している**（2026-09-05）。

        看板を読んで「打ちながら変えられる」と期待すると外れるので、
        その差をここに残す。
        ...
        **例外にしなかった理由**は、履歴に書式の1手が挟まると「置いたばかり
        のセリフを Esc で追加ごと取り消す」が効かなくなること（→ 要件定義
        6.5）。**打ちながら変えられないのは押し直せば済む。気づけない空の枠
        が残るのは、押し直しでは済まない。**
        """
        window = window_with_text
