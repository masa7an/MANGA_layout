...
class Test探す:
    ...
    def test_セリフの言い換えで当たる(self, entries):
        """前の呼び名「セリフ」で探しても「テキスト」に届く。

        2026-08-07 は逆向き（「テキスト」→「セリフ」）だった。2026-09-27 に
        画面の呼び名を「テキスト」へ改めたので（フキダシと取り違えるため）、
        読み替えの向きも入れ替えた。
        """
        hits = trails(search(entries, "セリフ"))
        assert "テキスト → テキストを追加 (T)" in hits
        assert hits == trails(search(entries, "テキスト"))
        assert hits == trails(search(entries, "せりふ"))
...
class Test無い機能に無いと答える:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("ルビ", "テキストの書式は、大きさ・太字・寄せ・縦横だけです"),
            ("行間", "テキストの書式は、大きさ・太字・寄せ・縦横だけです"),
            ("定規", "その機能はありません"),
        ],
    )
