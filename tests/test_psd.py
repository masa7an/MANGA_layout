...
def test_灰色などの中間値が続く所も大きく縮む():
    """以前は連続検出の種を 0x00 / 0xFF の2値だけに絞っていたため、
    灰色トーン（→ 要件定義 6.27）のような中間値のベタが続く場所は
    見つからず、リテラルのまま書かれてファイルが肥大していた
    （壊れてはいなかった。2026-08-08 に発見）。
    """
    assert len(packbits(bytes([166]) * 4096)) < 100
